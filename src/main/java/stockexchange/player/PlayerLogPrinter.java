...
public class PlayerLogPrinter {
	...
	public static void printOffers(String commentar, List<Offer> offer, Date date) {
		...
		for (int i = 0; i < offer.size(); i++) {
			log4j.log(Level.forName("NOTICE", 150), format.format(date) + " " + commentar + (i + 1) + ": "
					+ offer.get(i).getCompanyCode() + " " + offer.get(i).getAmount() + " " + offer.get(i).getPrice());
		}
	}
	...
	public static void printCosts(BigDecimal costOfShares, BigDecimal commision, BigDecimal toPay, Date date) {
		...
		log4j.log(Level.forName("NOTICE", 150),
				format.format(date) + " Cost of Share : " + costOfShares.setScale(2, RoundingMode.HALF_UP)
						+ " | Commission : " + commision.setScale(2, RoundingMode.HALF_UP) + " | To pay : "
						+ toPay.setScale(2, RoundingMode.HALF_UP));
	}
